<x-layout title="Admin Login - DelWell">

  <div class="min-h-[calc(100vh-200px)] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-md w-full space-y-8 bg-white p-10 rounded-xl shadow-lg border border-gray-200/80">
      <div>
        <h2 class="mt-6 text-center text-4xl font-extrabold text-[#3A3A3A] font-['Cormorant_Garamond']">Admin Portal</h2>
        <p class="mt-2 text-center text-sm text-gray-600">Sign in to access the admin dashboard.</p>
      </div>
      ...
      <form class="mt-8 space-y-6" action="{{ route('admin.authenticate') }}" method="post">
        ...
        <div>
          <button
            type="submit"
            class="group relative w-full flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-[#A3B18A] hover:bg-[#A3B18A]/90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#A3B18A] transition-colors duration-200"
          >
