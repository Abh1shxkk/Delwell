...
@section('title', 'Edit Waitlist Question')
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Edit Question: {{ $question->field_name }}</h3>
                ...
                <form method="POST" action="{{ route('admin.waitlist-questions.update', $question->id) }}">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                        @if ($errors->any())
                        ...
                        <div class="row">
                            <div class="col-md-8">
                                <div class="mb-3">
                                    ...
                                    @enderror
                                    <div class="form-text">
                                        <span id="char-count">{{ strlen($question->question_text) }}</span>/1000 characters
                                    </div>
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('question_type') is-invalid @enderror" 
                                        ...
                                        <option value="radio" {{ old('question_type', $question->question_type) == 'radio' ? 'selected' : '' }}>Radio Buttons</option>
                                        <option value="checkbox" {{ old('question_type', $question->question_type) == 'checkbox' ? 'selected' : '' }}>Checkboxes</option>
                                        <option value="email" {{ old('question_type', $question->question_type) == 'email' ? 'selected' : '' }}>Email Input</option>
                                    </select>
                                    @error('question_type')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                ...
                                <div class="mb-3" id="optionsContainer" style="display: {{ in_array($question->question_type, ['radio', 'checkbox']) ? 'block' : 'none' }};">
                                        ...
                                        @else
                                            @foreach($options as $index => $option)
                                                <div class="input-group mb-2 option-item">
                                                    ...
                                                    <button type="button" class="btn btn-danger remove-option" {{ $index === 0 ? 'disabled' : '' }}>
                                                        <i class="fas fa-times"></i>
                                                    </button>
                                                </div>
                                            @endforeach
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control @error('help_text') is-invalid @enderror" 
                                              ...
                                              maxlength="500">{{ old('help_text', $question->help_text) }}</textarea>
                                    @error('help_text')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="mb-3" id="maxSelectionsContainer" style="display: {{ $question->question_type == 'checkbox' ? 'block' : 'none' }};">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('is_required') is-invalid @enderror" 
                                            ...
                                            name="is_required" 
                                            required>
                                        <option value="1" {{ old('is_required', $question->is_required ? '1' : '0') == '1' ? 'selected' : '' }}>Yes - Required</option>
                                        <option value="0" {{ old('is_required', $question->is_required ? '1' : '0') == '0' ? 'selected' : '' }}>No - Optional</option>
                                    </select>
                                    @error('is_required')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('is_active') is-invalid @enderror" 
                                            ...
                                            name="is_active" 
                                            required>
                                        <option value="1" {{ old('is_active', $question->is_active ? '1' : '0') == '1' ? 'selected' : '' }}>Yes - Active</option>
                                        <option value="0" {{ old('is_active', $question->is_active ? '1' : '0') == '0' ? 'selected' : '' }}>No - Inactive</option>
                                    </select>
                                    @error('is_active')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
